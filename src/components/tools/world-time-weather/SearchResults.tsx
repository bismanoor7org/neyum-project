...
export function SearchResults({ data }: SearchResultsProps) {
  const placeLabel = formatLocationDisplayName(data);

  return (
    <div
      className="card-luxury p-6 md:p-8"
      aria-label={`Weather and time for ${placeLabel}`}
    >
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex items-center gap-2 text-foreground/50">
            <MapPin className="h-4 w-4 text-gold" strokeWidth={1.5} aria-hidden />
            <span className="text-xs font-medium uppercase tracking-[0.14em]">Search Result</span>
          </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat icon={Droplets} label="Humidity" value={formatHumidity(data.humidity)} />
        <Stat icon={Wind} label="Wind" value={formatWindSpeed(data.windSpeed)} />
        <Stat
          icon={Sunrise}
          label="Sunrise"
